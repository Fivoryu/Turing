...
void ListaSM::Insertar(int dir, int ele)
{
...
void ListaSM::Modifica(int dir, int ele)
{
	if (!Vacia())
...
void ListaSM::EliminarDato(int ele)
{
	int aux = PtrElementos;
	while (aux != -1)
	{
...
void ListaSM::Mostrar(TCanvas* canvas)
{
	...
	if (!Vacia())
	{
		...
		while (aux != -1)
		{
			int Dato = csMemoria.Obtener_Dato(aux, "->elemento");
			cad += IntToStr(Dato) + ",";
			num = csMemoria.Obtener_Dato(aux, "->sig");
			AnsiString cad2 = IntToStr(Dato);
			Dir += StrToInt(num);
